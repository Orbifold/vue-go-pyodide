<template>
    ...
    <div id="diagramHost" style="z-index: 2; border: 1px solid silver; border-radius:5px; width: 100%; height: 50vh; cursor: auto; background-color: rgba(255, 255, 255, 0.9)">
...
export default class DiagramView extends Vue {
  ...
  async createGraphModel() {
    ...
    this.diagram.clear()
    const nodeData = this.data.nodes.map((n) => {
      return {
        key: n.id,
        name: n.name
      };
    });
    const linkData = this.data.edges.map((e) => {
      return {
        from: e.sourceId,
        to: e.targetId,
        name: e.name
